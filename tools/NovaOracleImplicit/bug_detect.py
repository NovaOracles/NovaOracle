...
this tested sequence.
  ...
  In addition to checking page transitions and UI changes,
  independently check whether the GUI behavior at each step is
  consistent with all persistent states inferred from earlier interactions.

  3. Persistent State Changes: The persistent state changes need to be
...
screenshots to reason.
  ...
  behavior. - Determine whether the observed behavior is
  ...
  If any step has:
  "meets_expectation": false, or "state_behavior_consistency": false,
  then "bug_found" SHOULD be true.
...
“meets_expectation”: true, “state_behavior_consistency”: true,
“reason”: “…” } ], “bug_found”: true,
...
class BugDetectInterface:
    ...
    def get_output(self,image, pagepath):
